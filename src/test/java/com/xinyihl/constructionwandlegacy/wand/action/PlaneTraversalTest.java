...
public class PlaneTraversalTest {
    ...
    public void noLockPreservesLegacyBreadthFirstOrderIncludingDiagonals() {
        List<BlockPos> result = PlaneTraversal.traverse(ORIGIN, EnumFacing.UP, WandState.Lock.NOLOCK, 9, pos -> pos);
    ...
    public void countLimitTruncatesStableOrderWithoutExtraEvaluation() {
        ...
        List<BlockPos> result = PlaneTraversal.traverse(ORIGIN, EnumFacing.EAST, WandState.Lock.NOLOCK, 3, pos -> {
            evaluated.add(pos);
            return pos;
        });
    ...
    public void locksSelectTheExpectedAxisForFloorAndWallFaces() {
        assertEquals(Arrays.asList(pos(0, 0, 0), pos(0, 0, -1), pos(0, 0, 1), pos(0, 0, -2), pos(0, 0, 2)), PlaneTraversal.traverse(ORIGIN, EnumFacing.UP, WandState.Lock.NORTHSOUTH, 5, pos -> pos));

        assertEquals(Arrays.asList(pos(0, 0, 0), pos(0, 1, 0), pos(0, -1, 0), pos(0, 2, 0), pos(0, -2, 0)), PlaneTraversal.traverse(ORIGIN, EnumFacing.NORTH, WandState.Lock.VERTICAL, 5, pos -> pos));

        assertEquals(Collections.emptyList(), PlaneTraversal.traverse(ORIGIN, EnumFacing.UP, WandState.Lock.VERTICAL, 5, pos -> pos));
    }
    ...
    public void everyReachablePositionIsEvaluatedAtMostOnce() {
        ...
        List<BlockPos> result = PlaneTraversal.traverse(ORIGIN, EnumFacing.UP, WandState.Lock.NOLOCK, 100, pos -> {
    ...
    public void rejectedPositionDoesNotExpandTraversal() {
        ...
        List<BlockPos> result = PlaneTraversal.traverse(ORIGIN, EnumFacing.UP, WandState.Lock.NORTHSOUTH, 10, pos -> {
            evaluated.add(pos);
            return ORIGIN.equals(pos) ? pos : null;
        });

        assertEquals(Collections.singletonList(ORIGIN), result);
        assertEquals(new HashSet<>(Arrays.asList(ORIGIN, pos(0, 0, -1), pos(0, 0, 1))), evaluated);
    ...
    public void nonPositiveLimitDoesNotEvaluateOrigin() {
        ...
        assertEquals(Collections.emptyList(), PlaneTraversal.traverse(ORIGIN, EnumFacing.UP, WandState.Lock.NOLOCK, 0, pos -> {
            evaluated.add(pos);
            return pos;
        }));
