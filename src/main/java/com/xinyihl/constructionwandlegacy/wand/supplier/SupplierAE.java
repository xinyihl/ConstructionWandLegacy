...
public class SupplierAE extends SupplierInventory {
    private final HandlerAE aeHandler = new HandlerAE(player);
    ...
    protected void addBlockStack(ItemStack stack) {
        ...
        int count = aeHandler.countItems(player, normalized, null);
        if (count > 0) {
    ...
    public int takeItemStack(ItemStack stack) {
        ...
        return aeHandler.useItems(player, stack, count, null);
    }
