...
public final class PlaceOperation implements WandOperation {
    ...
    private PlaceOperation(BlockPos pos, ItemStack itemStack, ItemBlock item,
                           @Nullable IBlockState supportingBlock, boolean targetMode,
                           IBlockState previewState) {
        this.pos = pos;
    ...
    @Nullable
    public static PlaceOperation create(WandContext context, BlockPos pos, ItemStack itemStack,
        ...
        boolean targetMode = supportingBlock != null
                && context.getState().getDirection() == WandState.Direction.TARGET;
    ...
    public ApplyResult apply(WandContext context, WandPlan.ExecutionToken token) {
        ...
        try {
            IBlockState recalculated = calculateState(
                    context, pos, itemStack, item, supportingBlock, targetMode);
            if (recalculated == null) {
                return ApplyResult.rejected("placement is no longer valid");
            }
            if (!WandUtil.placeBlockAt(world, context.getPlayer(), pos, itemStack, recalculated,
                    context.getRayTraceResult())) {
                ...
                return ApplyResult.failedWithChange(
                        "placement rejection could not restore the original block", null,
                        new PlaceChange(pos, before, world.getBlockState(pos)), rollback);
            }
            ...
            return ApplyResult.failedWithChange("exception while placing block", exception,
                    new PlaceChange(pos, before, world.getBlockState(pos)), rollback);
        }
    }
    ...
    private static IBlockState calculateState(WandContext context, BlockPos pos, ItemStack itemStack,
                                               ItemBlock item, @Nullable IBlockState supportingBlock,
                                               boolean targetMode) {
        World world = context.getWorld();
        ...
        IBlockState state = block.getStateForPlacement(world, pos, facing, hitX, hitY, hitZ,
                itemStack.getMetadata(), player, EnumHand.MAIN_HAND);
        if (targetMode && supportingBlock != null) {
            ...
            for (IProperty property : state.getPropertyKeys()) {
                ...
                IProperty<?> source = sourceProperties.stream()
                        .filter(candidate -> candidate.getName().equals(property.getName()))
                        .findFirst()
                        .orElse(null);
                if (source == null) {
                    continue;
