...
public final class WandExecutor {
    ...
    ExecutionResult execute(@Nullable WandContext context, WandPlan plan, ExecutionAccess access) {
        ...
        try {
            ...
            if (applied.getStatus() != WandOperation.ApplyResult.Status.APPLIED
                    || applied.getChange() == null) {
                cancel(planned.reservation());
                if (applied.getStatus() == WandOperation.ApplyResult.Status.FAILED) {
                    logger.warn("Wand operation failed at {}: {}", operation.getPos(),
                            applied.getMessage(), applied.getCause());
                }
                if (applied.getChange() != null
                        && (applied.getRollbackResult() == null
                        || !applied.getRollbackResult().isRestored())) {
            ...
            try {
                ...
                recoverWorldThenRefund(change, rollbackReceipt(planned.reservation()),
                        operation.getPos(), access);
                index++;
                break;
            }
            if (receipt == null) {
            ...
            return succeeded > 0
                    ? ExecutionResult.success(succeeded, completed)
                    : ExecutionResult.failure("no wand operation succeeded");
        } finally {
    ...
    private void recoverWorldThenRefund(WandOperation.AppliedChange change,
                                        @Nullable MaterialReceipt receipt,
        ...
        if (receipt.getRemainingCount() > 0) {
            logger.error("Material rollback at {} still has {} undelivered item(s)",
                    pos, receipt.getRemainingCount());
            access.recordPending(change, receipt, true);
        }
    }

    private void logRecoveryFailure(net.minecraft.util.math.BlockPos pos,
    ...
    interface ExecutionAccess {
        ...
        void recordPending(WandOperation.AppliedChange change, @Nullable MaterialReceipt receipt,
    ...
    private final class ContextAccess implements ExecutionAccess {
        ...
        public boolean canContinue() {
            return !context.getMutableWand().isEmpty()
                    && context.getWandItem().remainingDurability(context.getMutableWand()) > 0;
        }
    ...
    public static final class ExecutionResult {
        ...
        private ExecutionResult(boolean success, int succeededOperations,
                                @Nullable WandTransaction transaction, @Nullable String failureReason) {
